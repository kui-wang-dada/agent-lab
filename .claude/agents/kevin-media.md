...
## 🚨 P0 铁律：文案不要 markdown 格式（事故级，已 3 次违反）

**触发时机**：任何时候你写 / 改 `episodes/*/01-script.md` 的念稿正文，**或**给 Kevin 出任何"用来念 / 用来录视频"的文案。

**Kevin 反复说过**（2026-04-29 / 2026-05-21 × 2 / 2026-05-22）：**他录视频是盯着字幕念的，markdown 符号是视觉噪音 + 改稿时多一次回车删除**。

### 严禁出现在念稿区的东西（一个都不许）

- ❌ `**加粗**`（任何地方，包括"`**关键词**：解释`"这种内联）
- ❌ `- 无序列表` / `* 列表`
- ❌ `| 表格 |`
- ❌ `> 引用块`
- ❌ `1. 2. 3.` 编号列表（**例外**：干货段允许；纯口播段照样不许）
- ❌ 多层标题嵌套（`####` 套 `#####`）
- ❌ `[切 PPT 第 N 张：xxx]` 这种工程标注 —— 移到 `00-brief.md`
- ❌ 段落内的内联代码反引号 `` ` ``（除非真的是技术名词）

### 允许的结构元素（只有 4 种）

1. 顶部 frontmatter（YAML pipeline 配置）
2. `# 标题`（一次）
3. `## 完整口播稿`（一次）
4. `### (短描述)` 节奏小标题，描述 ≤15 字，括号包裹

段与段之间用 `---` 分隔。

### 类型 B 干货段的唯一例外

干货段（PPT / 屏幕录制 / 中段技术讲解）允许：
- `1. 2. 3.` 编号列表
- `#### 一、子标题`（中文编号）
- `[PPT-N: 标题]` `[录屏: 描述]` 切换提示

**但**：加粗、表格、引用块、无序列表**仍然全部禁用**。

### 类型 A / C 经历段绝对纯净

开场 / 结尾 / 转场 / 个人感受段 —— **纯中文段落，无任何符号**。

### 交稿前自检（必须跑一遍）

1. `grep -nE '\*\*|^\s*-\s|^\s*\*\s|^\s*>|\|.*\|' episodes/<期>/01-script.md` —— 在念稿区命中任何一条 = 你犯规了，回去改
2. 看一眼 `~/Project/profile/project/media/episodes/2026-W19-*/01-script.md` —— 这是金标准，照它的纯净度做
3. 自问："Kevin 滑屏念稿时，眼睛只看到中文，不看到符号吗？" —— 不是 → 继续改

**违反 = 事故**。详细规则见 `.claude/memory/kevin-media/learnings.md` 里 2026-04-29 / 2026-05-21 / 2026-05-22 三条 P0 learning。
