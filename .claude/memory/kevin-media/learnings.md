...
## 2026-06-10 — 技术类自媒体脚本 = 抛观点 + 简证，无语气废话（默认写法）
**情境**：W23「AI 工程 OS 模板」v2 脚本 Kevin 反馈"整篇偏啰嗦"。技术类讲解的正确写法 = 抛出观点 → 一两句必要佐证就走，不绕。
就地重写后念稿区从约 4400 字（含标点）压到约 4450→实际砍到约 4449 字（纯中文 5278→3772），时长 ~17min 估算降到 ~12-13min，4 段结构 / 观点 / 关键佐证 / PPT 录屏标注全保留。
**经验**：Kevin 的技术段不要"教学口吻 + 情绪铺垫 + 过渡水词"。要删的是"说法的脂肪"，不是信息也不是段落：
- 删铺垫起手式："讲完痛点，讲讲我做这套东西时候脑子里真正在想的东西""现在我直接打开这套模板带你过一遍"——直接进结论
- 删自我评价水词："这部分是这套模板的灵魂""这是我跟很多人想法不一样的地方""我必须诚实不然就是骗你们"（保留一次即可，别每段都来）
- 删语气词冗余："其实""真的""到底""从来""分分钟"——口播段留极少量做节奏，干货段几乎清零
- 删过度礼貌/反复确认："建议你别快进"留一次够，"我先把话说在前面，免得有人觉得我在吹"压成"两个前提先说清"
- 合并同义重复句：原文常一个点说两遍（"减法比加法安全"+"砍东西比从头攒东西不容易出错"保留一句即可，这次保留了但其它处合并）
- 列表项开头直接抛结论名词，不要"第一个，xxx""第一件，xxx"的编号复述（`1.` 已经是第一了，正文别再"第一个"）
**写法公式**：每个点 = 一句结论 + 一两句必要佐证（为什么/堵什么坑/反直觉点），讲完就走。佐证超过两句就是在绕。
**下次怎么做**：写/改任何 Kevin 的技术类脚本（类型 B 干货段尤其），默认按这个密度写，不要等 Kevin 再说一遍"偏啰嗦"。
口播经历段（类型 A/C）保留情绪和真实感（陪儿子收尾、态度转变这类），但同样砍语气词和过渡水词。
**适用场景**：所有 01-script.md 撰写/修改，特别是技术讲解、模板拆解、原理说明类内容。与 2026-05-21 三段式 learning 配套：三段式管"结构"，这条管"密度/语言脂肪"。
