# Kevin Wang — User Model

> Hermes 风格的"deepening user model"。由 `kevin-curator` 每周日 21:30 自动维护。
> 所有 agent 工作前必读。结构：稳定 → 高频 → 低频 → 即时。

**Last updated**: 2026-05-11（手动初版，待 curator 接管）

---

## Identity（稳定，半年级别变化）

- **角色**：全栈 + AI 应用开发工程师，10 年+ 经验
- **职业身份**：Upwork 自由职业者（2024 起），承接海外客户
- **位置**：中国，远程办公
- **家庭**：两个儿子
- **背景转折**：35 岁兽医转行程序员
- **长期目标**：1-2 年维度建立"国内 + 全球"双向专业可信度

---

## Working Style（高频观察，月度更新）

### 协作偏好
- **极简路线**，反对过度设计；新方案先问"能不能不做 / 不做行不行"
- **不替 Kevin 做决策**，给"选项 + 推荐 + 理由"，让他能反驳
- **不堆方法论**，给具体可执行步骤
- **不写空话和过度礼貌性铺垫**

### 沟通
- 中文交流，技术术语保留英文
- 接受直接质疑（"为什么这样做"），不需要软化
- 完成任务后报告"改了哪些文件"（用相对路径），不要问"还需要做什么"

### 工程
- 不写 `any` / 不吞异常 / 不留无说明 TODO
- 没把握的依赖不引入
- 改前先 build / typecheck

---

## Current Focus（低频，季度更新）

### 2026-Q2
- **agent-lab**：搭建 Hermes 风格的个人 agent 体系（本项目）
- **media**：每周 1 期视频 + 配套图文（已成熟，国内平台为主）
- **Upwork**：维持 $15-25k/月，单价目标向 $50-80/h 推进
- **基础**：稳定收入 + 长期事业积累，**商业化非目标**

### 长期意图
- "把心里的故事讲出来"（源自《李献计》）
- 中短篇内容起步，**不为商业、暂不为动画服务**

---

## Hot Context（本周，curator 每周更新）

- **2026-W19**：搭建 agent-lab 的 Hermes 风格架构。之前 4 agent 版本因路径错误未能真正成长，本周重构为 9 agent + hooks + curator 周巡。

---

## Critical Constraints（合规/边界，永久）

- **国内自媒体内容不展示海外网站**（Upwork / GitHub.com / 英文官网截图）
- 海外平台（Upwork / GitHub / 英文版个人站）正常做
- 海外社交平台（YouTube / X / Medium / dev.to）现阶段不开

---

## Relationships to Other Files

- 详细技术偏好 → `.claude/memory/kevin-dev/facts.md`
- 业务偏好 → `.claude/memory/kevin-biz/facts.md`
- 媒体偏好 → `.claude/memory/kevin-media/facts.md`
- 业务规划全文 → `.claude/memory/business-plan.md`
- 简历事实 → `.claude/memory/profile/`

---

## Change Log（curator 自动追加）

- 2026-05-11: 初版手写（agent-lab 重构）
