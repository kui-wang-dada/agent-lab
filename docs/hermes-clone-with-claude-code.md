...
│    kevin-product   kevin-coder    kevin-qa                  │
│                                                             │
...
| kevin-coder | opus | 全栈实现 + 架构 + 契约 + ADR 一体；复杂任务自行 spawn 并行 subagent | shared (kevin-dev) |
| kevin-qa | sonnet | 测试、bug 复现、E2E、回归 | shared (kevin-dev) |

### 5.2 切分原则

- **按"心智模式"切而非按"技术栈"切**：`kevin-coder` 同时负责前端 + 后端 + 架构 + 契约——AI 没有"一个人不可能精通所有"的人类约束，prompt 装得下就不分；复杂任务现派 subagent
- **业务层按"语言"切而非"sales/delivery"切**：英文客户与中文客户的文化、合规、报价、合同条款差异大于销售期与交付期的差异
- **dev 类 agent 共享 facts**：技术偏好（不写 `any`、不吞异常等）对 product/coder/qa 通用，无需重复维护
...
测试用例：在主 session 发送 `@kevin-coder 我想给某项目加用户头像上传功能（含后端存储）。先告诉我你打算怎么处理这个任务，不要直接动手。`
...
4. **跨 cwd 限制**：项目级 agent 仅在 `agent-lab` cwd 下生效，其他项目目录无法直接 `@kevin-coder`。可通过 user-level 部署解决，但会失去项目隔离
...
| 含"代码 / 写代码 / 改 bug / 重构 / 架构 / 契约 / ADR / 前端 / Next.js / RN / Tailwind / 后端 / API / 数据库 / FastAPI / Node / Prisma / 集成" | kevin-coder |
